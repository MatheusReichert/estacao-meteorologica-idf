...
void sgp_task(void *pvParamters)
{
    ...
    ESP_ERROR_CHECK(hdc1000_init_desc(&hdc, 0x40, 0, CONFIG_EXAMPLE_I2C_MASTER_SDA, CONFIG_EXAMPLE_I2C_MASTER_SCL));    
    ...
    while (1)
    {
        ...
        ESP_ERROR_CHECK(hdc1000_measure(&hdc, &hdc_values.temperature, &hdc_values.humidity));

        // Feed it to SGP40
        int32_t voc_index;
        ESP_ERROR_CHECK(sgp40_measure_voc(&sgp, hdc_values.humidity, hdc_values.temperature, &voc_index));

        ESP_LOGI(TAG, "%.2f °C, %.2f %%, VOC index: %" PRIi32 ", Air is [%s]",
                hdc_values.temperature, hdc_values.humidity, voc_index, voc_index_name(voc_index));
