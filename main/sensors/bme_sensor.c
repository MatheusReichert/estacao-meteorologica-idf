...
void bme_init(void)
{
    ...
    ESP_ERROR_CHECK(bme680_init_desc(&sensor, ADDR, PORT, (gpio_num_t)CONFIG_EXAMPLE_I2C_MASTER_SDA, (gpio_num_t)CONFIG_EXAMPLE_I2C_MASTER_SCL));
        ...
        if (bme680_force_measurement(&sensor) == ESP_OK)
        {
            ...
            if (bme680_get_results_float(&sensor, &bmp_values) == ESP_OK)
            {
                ESP_LOGI(TAG, "BME680 Sensor: %.2f °C, %.2f %%, %.2f hPa, %.2f Ohm",
                       bmp_values.temperature, bmp_values.humidity, bmp_values.pressure, bmp_values.gas_resistance);
            }
        }
